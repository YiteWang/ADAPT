...
class ResGenerator32(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.linear = nn.Linear(args.latent_dim, 4 * 4 * 256)
...
class ResGenerator48(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.linear = nn.Linear(args.latent_dim, 6 * 6 * 512)
        self.blocks = nn.Sequential(
            GenBlock(512, 256),
            GenBlock(256, 128),
            GenBlock(128, 64),
...
class ResGenerator64(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.linear = nn.Linear(args.latent_dim, 4 * 4 * 1024)
