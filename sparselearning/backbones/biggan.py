...
# sn = partial(torch.nn.utils.parametrizations.spectral_norm, eps=1e-12)
sn = partial(torch.nn.utils.spectral_norm, eps=1e-12)
...
class Generator32(nn.Module):
    def __init__(self, args, z_dim=128, n_classes=10, ch=64):
        super().__init__()
...
class Generator64(nn.Module):
    def __init__(self, args, z_dim=128, n_classes=100, ch=80, shared_dim=128):
        super().__init__()
        channels_multipler = [16, 8, 4, 2, 1]
        ...
        self.blocks = nn.ModuleList([
            GenBlock(ch * 16, ch * 8, cbn_in_dim),  # ch*16 x 4 x 4
            GenBlock(ch * 8, ch * 4, cbn_in_dim),   # ch*16 x 8 x 8
            # GenBlock(ch * 4, ch * 2, cbn_in_dim),    # ch*8 x 16 x 16
            nn.ModuleList([                          # ch*4 x 16 x 16
                GenBlock(ch * 4, ch * 2, cbn_in_dim),
                Attention(ch * 2, True),             # ch*2 x 32 x 32
            ]),
            GenBlock(ch * 2, ch * 1, cbn_in_dim),    # ch*1 x 64 x 64
        ])

        self.output_layer = nn.Sequential(
            nn.BatchNorm2d(ch * 1),
            nn.ReLU(),
            sn(nn.Conv2d(ch * 1, 3, 3, padding=1)),  # 3 x 64 x 64
            nn.Tanh())
...
class Discriminator32(nn.Module):
    def __init__(self, args, n_classes=10, ch=64):
        super().__init__()
        self.fp16 = False
        ...
        self.classifier = nn.Linear(ch * 4, 1)
        self.embedding = sn(nn.Embedding(n_classes, ch * 4))
        res32_weights_init(self)

    def forward(self, x, y):
        h = self.blocks(x).sum(dim=[2, 3])
        h = self.classifier(h) + (self.embedding(y) * h).sum(dim=1, keepdim=True)
        return h


class Discriminator64(nn.Module):
    def __init__(self, args, n_classes=100, ch=80):
        super().__init__()
    ...
    def forward(self, x, y):
        h = self.blocks(x).sum(dim=[2, 3])
        print(self.blocks(x).shape,self.blocks(x).sum(dim=[2, 3]).shape, (self.embedding(y) * h).sum(dim=1, keepdim=True).shape)
...
if __name__ == '__main__':
    ...
    y = torch.tensor([1])
    G = Generator64('')
    print(G(input, y).shape)
    D = Discriminator64('')
    print(D(G(input, y), y).shape)
